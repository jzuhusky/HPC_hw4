...
/* Do the non-blocking send and receive operations */
  for (i=0; i<REPS; i++) {
    ...
    MPI_Wait(&reqs[offset+1], &stats[offset+1]); // Need to wait after Non-Block recv...
    offset += 2;
    if ((i+1)%DISP == 0)
...
/* Task 3 does the non-blocking receive operation */
  if (rank == 3) {
    ...
    offset = 0;
    for (i=0; i<REPS; i++) {
      MPI_Irecv(&buf, 1, MPI_INT, src, tag1, COMM, &reqs[offset]); // there needs to be a wait after this non-blcok recv
      MPI_Wait(&reqs[offset], &stats[offset]); // NOT at the end of the program...
      offset += 1;
      if ((i+1)%DISP == 0)
...
//MPI_Waitall(nreqs, reqs, stats); // get rid of this...
